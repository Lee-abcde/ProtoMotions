...
def compute_intermimic_finger_rotation_reward(
    body_rot: Tensor,
    ref_body_rot: Tensor,
    ...
    """Track bilateral parent-relative finger rotations."""
    ...
    return torch.exp(-(left_finger_cost + right_finger_cost))
...
def compute_intermimic_contact_reward(
    ...
    negative_cost = (object_contacts * (labels < 0).float()).mean(dim=-1)

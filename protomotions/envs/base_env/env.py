...
class BaseEnv:
    ...
    def _prepare_for_render(self):
        ...
        before_render = getattr(self.control_manager, "before_render", None)
    ...
    def step(self, action: Tensor):
        ...
        markers_callback = (
            ...
            if hasattr(self.control_manager, "before_render")
            else self.get_markers_state
        )
        self.simulator.step(processed_action, markers_callback=markers_callback)
    ...
    def post_physics_step(self):
        ...
        if self.state_history is not None:
            ...
            ground_heights = self.terrain.get_ground_heights(
                history_state.rigid_body_pos[
                    :, self.robot_config.anchor_body_index
                ]
            ).squeeze(-1)
        ...
        post_reward = getattr(self.control_manager, "post_reward", None)
        if post_reward is not None:
            post_reward(self.rew_buf)
    ...
    def _build_context(
        ...
        if current_state is None:
            ...
            raise ValueError("current_state and env_ids cannot be provided together")
        anchor_idx = self.robot_config.anchor_body_index
    ...
    def _build_scene_surface_context(
        ...
        object_contact_forces = self.simulator.get_object_contact_buf(env_ids)
    ...
    def _build_global_context(
        ...
        return self._build_context(current_state=current_state)
    ...
    def compute_ref_reset_state(
        ...
        new_object_states.root_ang_vel = torch.zeros_like(
    ...
    def reset(
        ...
        before_reset = getattr(self.control_manager, "before_reset", None)
        if before_reset is not None:
            before_reset(env_ids)
        ...
        if ref_env_ids.numel() == 0:
            ...
            modify_ref_reset_state = getattr(
                self.control_manager, "modify_ref_reset_state", None
            )
            if modify_ref_reset_state is not None:
                new_states, new_object_states = modify_ref_reset_state(
                    ref_env_ids,
                    motion_ids,
                    motion_times,
                    new_states,
            ...
            if modify_ref_reset_state is not None:
                ref_states, ref_object_states = modify_ref_reset_state(
                    ref_env_ids,
                    motion_ids,
                    motion_times,
                    ref_states,
    ...
    def get_state_dict(self):
        ...
        get_control_state = getattr(self.control_manager, "get_state_dict", None)
        if get_control_state is not None:
    ...
    def load_state_dict(self, state_dict):
        ...
        if "control_manager" in state_dict:
            load_control_state = getattr(
                self.control_manager, "load_state_dict", None
            )
            if load_control_state is not None:
                load_control_state(state_dict["control_manager"])

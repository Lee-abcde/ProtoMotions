...
class MimicControl(ControlComponent):
    ...
    def populate_context(self, ctx: EnvContext) -> None:
        ...
        history_steps = getattr(
            getattr(self.env, "config", None), "num_state_history_steps", 0
        )
        if history_steps > 0:
        ...
        get_text_override = getattr(
            self.env.motion_lib, "get_text_embedding_override", None
        )
        if get_text_override is None:
            override_text_embedding, override_valid_mask = None, None
        else:
            override_text_embedding, override_valid_mask = get_text_override(num_envs)
        ...
        elif getattr(self.env.motion_lib, "has_text_embeddings", lambda: False)():

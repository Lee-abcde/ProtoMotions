...
class PosteriorOnlyKLDistillModel(TextResidualMixin, BaseModel):
    """Gaussian posterior encoder-decoder without a deployable prior branch."""
    ...
    def forward(self, tensordict: TensorDict) -> TensorDict:
        ...
        privileged_std = torch.exp(0.5 * encoder_logvar)
        vae_noise = tensordict["vae_noise"]
        privileged_z = self.reparameterization(
            encoder_mu, privileged_std, vae_noise
        )
    ...
    def kl_loss(self, tensordict: TensorDict):
        ...
        return 0.5 * (encoder_mu.square() + encoder_var - encoder_logvar - 1)
    ...
    def calculate_aux_losses(
        ...
        encoder_mu = tensordict["encoder_mu"]
        encoder_logvar = tensordict["encoder_logvar"]
        mean_regu = encoder_mu.square().mean() * vae_config.prior_mean_regu_coeff
        logvar_regu = (
            encoder_logvar.square().mean() * vae_config.prior_logvar_regu_coeff
        )
        total = (mean_regu + logvar_regu) * vae_config.prior_regu_weight

        log_dict = {
            "distill/encoder_mean_regu": mean_regu.detach(),
            "distill/encoder_logvar_regu": logvar_regu.detach(),
            "distill/encoder_regu_loss": total.detach(),
        }
        self._add_text_residual_log_dict(tensordict, log_dict)
        return total, log_dict

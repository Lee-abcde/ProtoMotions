...
#
"""VQ-PAE variant that uses both future max-coords observations and target poses."""
...
_TRANSFORMER_MODULE = _load_sibling_module(
    "transformer.py", "masked_mimic_transformer_experiment_v2"
)
_BASE_VQPAE_MODULE = _load_sibling_module("vq_pae.py", "masked_mimic_vq_pae_base_v2")
...
def agent_config(
    ...
    target_pose_dim = num_bodies * (3 + 3 + 6 + 6 + 3 + 3)
    future_obs_dim = current_obs_dim + target_pose_dim
    ...
    preprocessor_config = ModuleContainerConfig(
        ...
        models=[
            ...
            ObsProcessorConfig(
                in_keys=["future_max_coords_obs", "vq_pae_target_poses"],

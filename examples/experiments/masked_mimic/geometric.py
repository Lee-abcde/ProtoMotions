...
#
"""Geometric primitive distillation experiment.

Uses a learned ellipse primitive codebook to explain local 3-point motion
segments and conditions the student policy on the predicted next-step latent.
"""
...
TOTAL_STORED_HISTORICAL_STEPS = max(
    2, _TRANSFORMER_MODULE.TOTAL_STORED_HISTORICAL_STEPS
)
...
def env_config(robot_cfg: RobotConfig, args: argparse.Namespace) -> EnvConfig:
    ...
    observation_components = {
        ...
        "geometric_target_poses": mimic_target_poses_max_coords_factory(
            with_velocities=True,
            future_steps=1,
        ),
...
def agent_config(
    ...
    preprocessor_config = ModuleContainerConfig(
        ...
        models=[
            ...
            ObsProcessorConfig(
                in_keys=["geometric_target_poses"],
                out_keys=["geometric_target_poses_norm"],
                normalize_obs=True,
                norm_clamp_value=5,
                module_operations=[ModuleOperationForwardConfig()],
            ),
    ...
    model_config = DistillGeometricModelConfig(
        prior_in_keys=[
            "max_coords_obs_norm",
            "historical_pose_obs_norm",
        ],
        ...
        trunk=trunk_config,
        num_historical_conditioned_steps=2,

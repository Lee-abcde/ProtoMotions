...
#
"""VQ-PAE variant with future-obs reconstruction and root mimic conditioning."""
...
def env_config(robot_cfg: RobotConfig, args: argparse.Namespace) -> EnvConfig:
    ...
    observation_components = {
        ...
        "historical_pose_obs": historical_max_coords_obs_factory(
            local_obs=True,
            root_height_obs=True,
            observe_contacts=False,
            history_steps=total_stored_historical_steps,
        ),
        "root_target_rot": MdpComponent(
            compute_func=build_target_root_rot,
            dynamic_vars={
                "current_state_root_rot": EnvContext.current.root_rot,
                "mimic_ref_anchor_rot": EnvContext.mimic.future_anchor_rot,
            },
            static_params={"future_steps": 1, "w_last": True},
        ),
        "root_target_xy": MdpComponent(
            compute_func=build_target_xy_offset,
            dynamic_vars={
        ...
        "root_target_height": MdpComponent(
            compute_func=build_target_height,
            dynamic_vars={
                "mimic_ref_anchor_pos": EnvContext.mimic.future_anchor_pos,
            },
            static_params={"future_steps": 1},
        ),
        "root_target_vel": MdpComponent(
            compute_func=build_target_root_vel,
            dynamic_vars={
                "current_state_anchor_rot": EnvContext.current.anchor_rot,
                "mimic_ref_root_vel": EnvContext.mimic.future_anchor_vel,
            },
            static_params={"future_steps": 1, "w_last": True},
        ),
        "root_target_ang_vel": MdpComponent(
            compute_func=build_target_root_ang_vel,
            dynamic_vars={
                "current_state_anchor_rot": EnvContext.current.anchor_rot,
                "mimic_ref_root_ang_vel": EnvContext.mimic.future_anchor_ang_vel,
            },
            static_params={"future_steps": 1, "w_last": True},
        ),
    }
    ...
    preprocessor_config = ModuleContainerConfig(
        ...
        models=[
            ...
            ObsProcessorConfig(
                ...
                out_keys=["root_mimic_target_norm"],
                normalize_obs=True,
                norm_clamp_value=5,
                module_operations=[ModuleOperationForwardConfig()],
            ),

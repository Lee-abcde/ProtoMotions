...
#
"""Simple latent flow distillation config with BM-style domain randomization."""
...
FLOW_FUTURE_STEPS = [1, 2, 4, 8]
TOTAL_STORED_HISTORICAL_STEPS = 5
NUM_HISTORICAL_CONDITIONED_STEPS = 5
CONTROL_FUTURE_STEPS = max(FLOW_FUTURE_STEPS)
...
def env_config(robot_cfg: RobotConfig, args: argparse.Namespace) -> EnvConfig:
    ...
    observation_components = {
        ...
        "encoder_future_target_obs": MdpComponent(
            ...
            static_params={"future_steps": FLOW_FUTURE_STEPS, "w_last": True},
        ),
        ...
        "text_embedding_obs": MdpComponent(
            ...
            dynamic_vars={"text_embedding": EnvContext.mimic.text_embedding},
        ),
        ...
        "noisy_mimic_reduced_coords_target_poses": make_reduced_target_pose_component(
            ...
            MdpComponent,
            use_noisy=True,
            future_steps=FLOW_FUTURE_STEPS,
        ),
...
def agent_config(
    ...
    target_relative_rot_dim = 6 * len(FLOW_FUTURE_STEPS)
    ...
    model_config = DistillFlowPolicyModelConfig(
        prior_in_keys=[
            "max_coords_obs_norm",
            "historical_pose_obs_norm",
            "text_embedding_obs_norm",
        ...
        trunk=trunk_config,
        future_steps=list(FLOW_FUTURE_STEPS),
        inference_horizon=1,
        ...
        target_relative_rot_dim=target_relative_rot_dim,
        latent_dim=current_obs_dim,
        history_embedding_dim=64,
        text_embedding_dim=64,
        future_embedding_dim=128,
        target_rot_embedding_dim=32,
        horizon_embedding_dim=16,
        history_dropout_prob=0.4,
        normalize_pose_obs=True,
        pose_norm_clamp_value=5.0,
        encoder_layers=[
            MLPLayerConfig(units=512, activation="silu"),
            MLPLayerConfig(units=256, activation="silu"),
        ],
        history_layers=[MLPLayerConfig(units=256, activation="silu")],
        prior_layers=[
            MLPLayerConfig(units=512, activation="silu"),
            MLPLayerConfig(units=256, activation="silu"),
        ],
        ...
        losses=FlowPolicyLossConfig(
            posterior_target_weight=1e-4,
            prior_posterior_weight=1e-4,
            next_latent_weight=1e-4,
            latent_regularization_weight=1e-6,
            prior_bc_weight=0.2,
        ),

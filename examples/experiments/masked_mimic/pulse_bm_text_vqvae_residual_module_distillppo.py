...
"""Residual PPO finetuning for text-conditioned PULSE VQ-VAE prior actions."""
...
def additional_experiment_arguments(parser: argparse.ArgumentParser):
    ...
    parser.add_argument(
        "--use-decoder-action-residual",
        action="store_true",
        help=(
            "Add a text/state/base-action residual head on top of the VQ decoder "
            "action."
        ),
    )
    parser.add_argument(
        "--decoder-action-residual-hidden-dim",
        type=int,
        default=1024,
        help="Hidden size for the decoder action residual MLP.",
    )
    parser.add_argument(
        "--decoder-action-residual-scale",
        type=float,
        default=0.02,
        help="Scale multiplied with the decoder action residual before addition.",
    )
    parser.add_argument(
        "--critic-expert-path",
        type=str,
...
def _vq_distill_agent_config(
    ...
    use_decoder_action_residual = bool(
        getattr(args, "use_decoder_action_residual", False)
    )
    action_residual_config = None
    if use_decoder_action_residual:
        action_residual_hidden_dim = int(
            getattr(args, "decoder_action_residual_hidden_dim", 1024)
        )
        action_residual_config = ModuleContainerConfig(
            in_keys=[
                "noisy_reduced_coords_obs",
                "historical_previous_processed_actions",
                "decoder_action",
            ...
            out_keys=["decoder_action_residual"],
            models=[
                ObsProcessorConfig(
                    in_keys=[
                        "noisy_reduced_coords_obs",
                    ...
                    out_keys=["action_residual_motion_obs_norm"],
                    normalize_obs=True,
                    norm_clamp_value=5,
                    module_operations=[ModuleOperationForwardConfig()],
                ),
                MLPWithConcatConfig(
                    in_keys=[
                        "action_residual_motion_obs_norm",
                        "text_embedding_obs",
                    ],
                    out_keys=["decoder_action_residual"],
                    num_out=robot_config.number_of_actions,
                    layers=[
                        MLPLayerConfig(
                            units=action_residual_hidden_dim,
                            activation="relu",
                        )
                        for _ in range(2)
                    ],
    ...
    model_config = VQDistillModelConfig(
        ...
        action_residual=action_residual_config,
        reconstruction=None,
        ...
        action_residual_scale=float(
            getattr(args, "decoder_action_residual_scale", 0.02)
        ),
...
def _actor_in_keys(model_config: VQDistillModelConfig):
    keys = []
    ...
    if model_config.action_residual is not None:
        base_action_key = getattr(
            model_config, "action_residual_base_action_key", "decoder_action"
        )
        keys.extend(
            key
            for key in model_config.action_residual.in_keys
...
def agent_config(
    ...
    if bool(getattr(args, "use_categorical_prior_transformer", False)):
        raise ValueError(
            "pulse_bm_text_vqvae_residual_module_distillppo.py does not support "
    ...
    args.use_decoder_action_residual = True
    distill_cfg = _vq_distill_agent_config(robot_config, env_config, args)
    vq_model_config = distill_cfg.model
    vq_model_config.train_categorical_prior_only = False
    vq_model_config.train_action_residual_only = True
    vq_model_config.load_categorical_prior_parameters = True
    vq_model_config.soft_code_target.enabled = False
    distill_cfg.evaluator.use_privileged_action_for_interaction = False
    ...
    return DistillPPOAgentConfig(
        ...
        action_loss_coef=0.0,
        action_loss_schedule=ActionLossScheduleConfig(enabled=False),
        ppo_loss_schedule=PPOLossScheduleConfig(enabled=False),
        mini_epoch_schedule=MiniEpochScheduleConfig(enabled=False),
        actor_lr_schedule=ActorLRScheduleConfig(enabled=False),
